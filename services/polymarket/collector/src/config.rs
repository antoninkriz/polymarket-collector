...
mod tests {
    ...
    fn env_bool_parses_truthy_values() {
        ...
        for v in ["true", "True", "TRUE", "1", "yes", "YES"] {
            std::env::set_var("TEST_ENV_BOOL", v);
            assert!(env_bool("TEST_ENV_BOOL"), "expected truthy for {v}");
        }
    ...
    fn env_bool_parses_falsy_values() {
        ...
        for v in ["false", "0", "no", "anything-else", ""] {
            std::env::set_var("TEST_ENV_BOOL_FALSE", v);
            assert!(!env_bool("TEST_ENV_BOOL_FALSE"), "expected falsy for {v}");
        }
        std::env::remove_var("TEST_ENV_BOOL_FALSE");
        assert!(!env_bool("TEST_ENV_BOOL_FALSE"));
    }
}
