...
use anyhow::Result;
use polymarket_collector::events::Event;
use polymarket_collector::lease::{PublisherLease, PublisherLeaseConfig};
use polymarket_collector::record::{CollectorContext, EventRecord};
...
async fn lease_is_exclusive_renewed_and_fences_stale_appends() -> Result<()> {
    ...
    let stale_sink = RedisStreamPublisher::connect(RedisStreamPublisherConfig {
        redis_url: REDIS_URL.into(),
        stream: stream.clone(),
